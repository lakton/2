...
def close_tcp_connection(src_ip, dst_ip, src_port, dst_port, interface):
    ...
    ack_pkt = Ether()/IP(src=dst_ip, dst=src_ip)/TCP(sport=dst_port, dport=src_port, flags='A', ack=1)
    ...
    last_ack_pkt = Ether()/IP(src=src_ip, dst=dst_ip)/TCP(sport=src_port, dport=dst_port, flags='A', ack=1)
...
def run_tests():
    ...
    sendp(http_post_req_sdnithub, iface="h1-eth0")
    time.sleep(1)
    close_tcp_connection("100.0.0.10", sdnithub_ip, 80, 80, "h1-eth0")
    ...
    sendp(http_post_req, iface="h1-eth0")
    time.sleep(1)
    close_tcp_connection("100.0.0.10", "100.0.0.45", 80, 80, "h1-eth0")
