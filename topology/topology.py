...
if __name__ == "__main__":
    ...
    net.get("ws1").cmd("python3 -m http.server 80 --bind 100.0.0.20")
    net.get("ws2").cmd("python3 -m http.server 80 --bind 100.0.0.21")
    net.get("ws3").cmd("python3 -m http.server 80 --bind 100.0.0.22")
    net.get("h1").cmd("cp /dev/null /etc/resolv.conf")
    net.get("h1").cmd("echo 'nameserver 100.0.0.25' > /etc/resolv.conf")
    net.get("h2").cmd("cp /dev/null /etc/resolv.conf")
    net.get("h2").cmd("echo 'nameserver 100.0.0.25' > /etc/resolv.conf")
    net.get("h3").cmd("cp /dev/null /etc/resolv.conf")
    net.get("h3").cmd("echo 'nameserver 100.0.0.25' > /etc/resolv.conf")
    net.get("h4").cmd("cp /dev/null /etc/resolv.conf")
    net.get("h4").cmd("echo 'nameserver 100.0.0.25' > /etc/resolv.conf")
    # Запуск сети
    net.start()
