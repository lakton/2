    ...
    while buf_len - offset >= 8: # 8 bytes is minimum OF message size
      ...
      if self.buf[offset] != of.OFP_VERSION:
        if ofp_type == of.OFPT_HELLO:
          ...
          log.warning("Bad OpenFlow version (0x%02x) on connection %s. Buffer content: %s, Offset: %d"
                      % (self.buf[offset], self, self.buf, offset))
...
class OpenFlow_01_Task (Task):
  ...
  """
  def __init__ (self, port = 6633, address = '0.0.0.0',#test host
